...
/**
 * Accumulator for a batched push. When a `SavingContext` is passed to
 * {@link set}, the changed USP/USL strings are added to `usls` instead of being
 * broadcast immediately; the batch driver (e.g. a future `stow`) posts once when
 * every write in the batch has completed.
 */
 ...
 */
function evaluatePath(root: any, parts: string[]): { target: any; lastKey: string } {
    let current = root;
...
function chainParts(path: string | null): string[] {
 ...
 */
function notify(usp: string, usl: string, ctx?: SavingContext): void {
    const msgs = usp === usl ? [usp] : [usp, usl];
    if (ctx !== undefined) {
        for (const m of msgs) ctx.usls.add(m);
    } else {
...
function setWebStorage(store: Storage, key: string, parts: string[], val: any): void {
    if (parts.length === 0) {
...
function setCookie(name: string, val: any): void {
...
function setHash(key: string, val: any): void {
    const params = new URLSearchParams(location.hash.slice(1));
    params.set(key, typeof val === 'string' ? val : JSON.stringify(val));
    location.hash = params.toString();
}

async function setIndexedDB(key: string, parts: string[], val: any): Promise<void> {
...
export async function set(usl: string, val: any, ctx?: SavingContext): Promise<void> {
    ...
    switch (protocol) {
        ...
        case 'sessionStorage':
            ...
            break;
        case 'cookie':
            if (parts.length > 0) throw new Error(`cookie protocol does not support an accessor chain: "${usl}"`);
            setCookie(key, val);
            break;
        case 'locationHash':
            if (parts.length > 0) throw new Error(`locationHash protocol does not support an accessor chain: "${usl}"`);
            setHash(key, val);
            break;
        case 'indexedDB':
